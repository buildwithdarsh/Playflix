...
export default function EmojiBar() {
  ...
  return (
    <div className="flex items-center justify-center gap-3 py-2 bg-black/60">
      {REACTIONS.map((r) => (
        <button
          key={r.key}
          onClick={() => sendReaction(r.key)}
          className="w-10 h-10 rounded-full bg-white/8 flex items-center justify-center text-white/70 active:scale-110 active:text-white transition-all hover:bg-white/15"
        >
          {r.icon}
        </button>
      ))}
